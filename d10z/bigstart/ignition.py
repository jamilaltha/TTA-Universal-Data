...
def check_ignition_condition(network: NodalNetwork,
                             threshold: float = PHI_IGNITION) -> Tuple[bool, float]:
    """
...
def trigger_ignition(network: NodalNetwork,
    ...
    Z = network.z_array
    mean_phase = cmath.phase(sum(Z))
    amplitudes = [abs(z) for z in Z]

    # All nodes align to mean phase (coherence → 1)
    network.z_array = [amp * cmath.exp(1j * mean_phase) for amp in amplitudes]

    # Small expansion perturbation (universe begins expanding)
    perturbation = [random.gauss(0.0, 0.01) for _ in range(len(Z))]
    new_phases = [mean_phase + delta for delta in perturbation]
    network.z_array = [amp * cmath.exp(1j * phase) for amp, phase in zip(amplitudes, new_phases)]
...
def prepare_for_ignition(network: NodalNetwork,
                        steps: int = 1000,
                        dt: float = 0.1,
                        threshold: float = PHI_IGNITION) -> bool:
    """
    ...
    for step in range(steps):
        ...
        if step % 100 == 0:
            status = "(critical)" if phi >= PHI_CRITICAL else ""
            print(f"  Step {step}: Φ = {phi:.4f} {status}")
...
def simulate_big_start(seed: Optional[int] = None) -> BigStartEvent:
    ...
    print(f"[D10Z] Creating Flower of Life ({FLOWER_OF_LIFE_NODES} nodes)...")
    network = create_flower_of_life(scale=1.0, seed=seed)
    print(f"  Initial state: {network}")
    
    # Prepare for ignition
    ready = prepare_for_ignition(network, steps=2000, dt=0.05, threshold=PHI_IGNITION)
